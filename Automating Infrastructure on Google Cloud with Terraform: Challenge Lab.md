...
```
```bash
export BUCKET_TECHPRO=
```
```bash
export INSTANCE_TECHPRO=
```
```bash
export VPC_TECHPRO=
```
...
resource "google_compute_instance" "'"$INSTANCE_TECHPRO"'" {
  name         = "'"$INSTANCE_TECHPRO"'"
...
terraform init
terraform apply -auto-approve
terraform taint module.instances.google_compute_instance.$INSTANCE_TECHPRO
terraform plan
terraform apply -auto-approve
...
resource "google_compute_firewall" "tf-firewall"{
  name    = "tf-firewall"
 network = "projects/'$PROJECT_ID'/global/networks/'$VPC_TECHPRO'"
